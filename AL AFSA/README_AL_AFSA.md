...
**No browser is required.** Each page is a classic ASP page that renders a Bootstrap 3
accordion (`div.panel-group` > `div.panel.panel-default`). The panel *bodies* — including
every address, phone and licence line — are present in the initial HTML response. v2 drove
DrissionPage and clicked each `h4` to expand panels; that is unnecessary and was removed.
v3 uses plain `requests` + BeautifulSoup.
...
This produced 28 real addresses out of 46 rows.
...
- Behaviour of this scraper on the Windows control server — it was written and executed on
  macOS with the global `python3` only.
