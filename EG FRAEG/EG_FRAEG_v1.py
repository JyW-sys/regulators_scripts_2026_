...
#   tempfolder/translation_cache.json so re-runs are byte-stable.
...
def _provider_translate(text):
    """Google via deep_translator. Swap this body to change provider."""
    global _translator
    ...
    for attempt in range(retries):
        try:
            ...
            if attempt == retries - 1:
                print("   !! translation failed for {!r}: {}".format(src[:40], exc))
