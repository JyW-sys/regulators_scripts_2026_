...
v1.5 changes vs v1.4 -- ONE BUG, the production 503:
  ...
  Measured on the live site 2026-08-13, through the same browser channel the
  production box was using when it failed:

    * ubi_no=01989895 is NOT a broken record.  On its own it returns HTTP 200
      and parses: "QBE Hongkong & Shanghai Insurance Limited".
    * it is entry 112 of 162, i.e. the failure was two thirds of the way
      through the detail loop, not on the first call.
    * all 162 detail calls back to back, no pacing, took 6.1 s and returned
      HTTP 200 every time -- histogram {200: 162}.

  So the URL is good and the request pattern is not inherently refused.  What
  the production box met was a TRANSIENT refusal: one request out of 162 came
  back 503 (Cloudflare serves rate-limited and re-challenged sub-requests that
  way, and that box has already shown it gets challenged far harder than the
  dev Mac -- it produced the v1.3 403 and the v1.4 ContextLostError).

  The defect is not that a 503 happened -- over 162 calls a refusal is a
  matter of time, and the site is under no obligation to answer all of them.
  The defect is v1.4's reaction to it:

    * ANY non-200 was fatal, immediately, with no retry and no backoff, so a
      single blip threw away a run that had already done 111 detail pages;
    * the body of the refusal was discarded, so the error could not say
      whether it was a Cloudflare challenge or the origin being down;
    * and one unreadable insurer took the whole list with it, when 15 of that
      insurer's fields are already present in the list API response.

  v1.5:
    * browser-side reads retry on transient statuses (403/408/429/5xx and a
      network-level failure) up to BROWSER_ATTEMPTS times with exponential
      backoff, re-clearing Cloudflare from attempt 2 onward;
    * once any retry has been needed, a small inter-call throttle switches on
      for the rest of the run, so a rate-limited box stops sprinting into the
      same wall;
    * the failing body is captured and quoted in the error, and a challenge
      body is named as such;
    * if a detail page is still unreadable after all attempts the insurer is
      written from the list API instead of aborting -- the row count stays 162
      and every degraded row is listed loudly at the end of the run.  It is
      never silently dropped and never silently pretended complete.
...
print("Running {} Web Scraping Tool v.1.5".format(regulatorName))
...
# List 1 makes 162 detail calls back to back.  Over that many requests an
# occasional refusal is a certainty, not an anomaly -- the production box hit
# HTTP 503 on call 112 of 162 and v1.4 threw the whole run away.  These
# statuses are treated as "ask again", not "give up": 403/503 are what
# Cloudflare returns for a re-challenge or a rate limit, 429 is an explicit
# rate limit, 5xx is the origin, and 0/-1 is the XHR failing below HTTP.
RETRY_STATUSES = (0, -1, 403, 408, 429, 500, 502, 503, 504)
BROWSER_ATTEMPTS = 5
RETRY_BASE_DELAY = 3          # seconds, doubled each attempt: 3, 6, 12, 24
THROTTLE_AFTER_RETRY = 0.5    # seconds between calls once anything has failed
...
try { xhr.send(null); }
catch (e) { return JSON.stringify({status: -1, body: '' + e, b64: ''}); }
...
class Channel(object):
    ...
    def __init__(self, page, session):
        ...
        self.mode = 'requests'
        self.switch_reason = ''
        self.throttle = 0.0     # raised the first time anything has to be retried
        self.retries = 0
    ...
    def _browser_fetch(self, script, url, timeout):
        """One browser-side XHR, retried through a transient refusal.

        v1.4 raised on the first non-200 and that is what killed the
        production run: HTTP 503 on detail call 112 of 162 discarded 111
        insurers already read.  Measured here the same day, all 162 calls
        returned 200 in 6.1 s -- so a refusal is not a property of the URL, it
        is weather.  Ask again, backing off, and re-clear Cloudflare from the
        second attempt in case the refusal was a re-challenge.
        """
        ...
        for attempt in range(1, BROWSER_ATTEMPTS + 1):
            ...
            payload = self._run(script, url, timeout)
            status = payload.get('status')
            if status == 200:
                if attempt > 1:
                    print('    recovered on attempt {} of {}'.format(attempt, BROWSER_ATTEMPTS))
                return payload

            # Keep the evidence.  v1.4 discarded the body, so its error could
            # not say whether 503 meant "Cloudflare re-challenge" or "origin
            # down" -- two different problems with two different fixes.
            ...
            if not self.throttle:
                # Something is refusing us.  Stop sprinting for the rest of
                # the run -- 162 calls in 6 s is exactly the shape of traffic
                # a rate limiter exists to stop.
                self.throttle = THROTTLE_AFTER_RETRY
                print('  [!] {} on {} -- backing off and throttling every later '
                      'call by {}s'.format(detail, url, THROTTLE_AFTER_RETRY))
            ...
            if attempt >= 2:
                # A refusal that survives one backoff may be Cloudflare having
                # re-armed the challenge on this origin; re-solve it.
                try:
                    load_and_clear(self.page, INSURERS_PAGE)
                except Exception as err:
                    print('    (re-clearing the origin failed: {})'.format(
                        type(err).__name__))

        raise RuntimeError(
            'the browser itself got {} for {} after {} attempt(s){}'.format(
                detail, url, attempt,
                '' if attempt == BROWSER_ATTEMPTS
                else ' -- not a retryable status, so it was not asked again'))

    def _browser_text(self, url):
        return self._browser_fetch(SYNC_XHR_TEXT, url, 180).get('body') or ''

    def _browser_binary(self, url):
        payload = self._browser_fetch(SYNC_XHR_BINARY, url, 600)
...
try:
    ...
    for position, entity in enumerate(insurers, 1):
        ...
        list_name_en = clean(entity.get('en_name'))

        # The list API already carries name, insurer type, business nature and
        # authorization year; the detail page adds address and the contact
        # fields.  So an unreadable detail page costs a few columns on ONE row
        # -- it is not worth losing the other 161 insurers over, which is what
        # v1.4 did on a single 503.  Degrade, count it, and shout at the end.
        try:
            detail = channel.json(INSURERS_DETAIL_API.format(ubi)).get('data') or {}
        except Exception as err:
            detail = {}
            detail_failures.append((list_name_en, ubi, str(err)[:160]))
            print('  [!] detail page for {} ({}) unreadable after retries -- writing '
                  'the row from the list API; address/phone/fax/website/email will '
                  'be blank on it'.format(list_name_en[:45], ubi))
        ...
        append_row({
            'Name': clean(detail.get('en_name')) or list_name_en,
            'InternalID_1': ubi,
...
print('\nTransport used: {}{}'.format(
    channel.mode,
    '' if channel.mode == 'requests' else ' (requests refused: {})'.format(channel.switch_reason)))
print('Browser-side reads that had to be retried: {}{}'.format(
    channel.retries,
    '' if not channel.retries else '  (throttle was switched on)'))

if detail_failures:
    print('\n!! {} insurer(s) were written WITHOUT their detail page after {} attempts'
          ' each. Name / type / business nature / authorization year are correct on'
          ' these rows; address, phone, fax, website and email are blank. Re-run to'
          ' pick them up, or deliver knowing which rows are short:'.format(
              len(detail_failures), BROWSER_ATTEMPTS))
    for name, ubi, err in detail_failures:
        print('   {:<50} ubi={} {}'.format(name[:50], ubi, err))
