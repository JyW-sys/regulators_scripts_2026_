...
#
# --- v4 changes --------------------------------------------------------------
# * Browser fallbacks now try Chrome first, then Edge (preferring the repo's
#   edgedriver_win64/msedgedriver.exe). The control server has no Chrome, so
#   v3's Chrome-only List 1 pager fallback died at startup there and List 1
#   degraded to page 1 only (10 of 20 banks).
# * After the SQL-Ready xlsx is written, one PDF per ListCode is generated in
#   tempfolder (entity names, one per line) -- same deliverable as
#   IT CONSOB / HU CBH: "<xlsx basename>- <ListCode>.pdf".
# ------------------------------------------------------------------
import os
import re
import time
...
print(f"Running {regulatorName} Web Scraping Tool v.4.0")
...
# Edge driver bundled at the repo root (the control server has Edge, no Chrome).
EDGE_DRIVER = os.path.join(os.path.dirname(scriptfolder),
                           "edgedriver_win64", "msedgedriver.exe")


def start_headless_browser(prefs=None):
    """Start a headless browser: Chrome first, then Edge. The control server
    has no Chrome -- only Edge plus the repo's edgedriver_win64 driver -- so a
    Chrome-only fallback dies at startup there. Returns None if neither starts.
    prefs (optional) = Chromium experimental prefs, e.g. download settings."""
    try:
    ...
    try:
        return webdriver.Chrome(options=configure(webdriver.ChromeOptions()))
    except Exception as e:
        print(f"   Chrome unavailable ({e}); trying Edge")
    try:
        opts = configure(webdriver.EdgeOptions())
        if os.path.exists(EDGE_DRIVER):
            from selenium.webdriver.edge.service import Service
            return webdriver.Edge(service=Service(EDGE_DRIVER), options=opts)
        return webdriver.Edge(options=opts)
    except Exception as e:
        print(f"   Edge unavailable too: {e}")
        return None


def scrape_list1_selenium():
    """Browser fallback for the Directory of Banks pager: drive a real browser
    (Chrome, or Edge on the control server), turn the numeric pager (the
    genuine __doPostBack the server accepts), and read the table off each page.
    Used when the requests-based POST pager is blocked on the control server.
    Dedupes by Name across pages."""
    try:
    ...
    if driver is None:
        print("   could not start any browser for List 1 fallback")
        return []
...
def _fetch_pdf_selenium(candidates, dest, landing_url=None, wait=600):
    """Browser fallback for the flaky cbj.gov.jo PDFs: a fresh headless browser
    session (Chrome, or Edge on the control server) visits the landing page
    first (the server is session/referer sensitive) then navigates to the PDF
    URL, saving it into tempfolder."""
    driver = start_headless_browser(prefs={
    ...
    if driver is None:
        print("   could not start any browser for browser download")
        return False
